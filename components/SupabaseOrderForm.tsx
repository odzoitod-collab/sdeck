...
const SupabaseOrderForm: React.FC<Props> = ({ orderId, onSubmit, onBack }) => {
  ...
  const inputClass = "w-full min-h-[50px] px-4 py-3 rounded-xl border border-gray-200 bg-white focus:border-[#8DC63F] focus:ring-3 focus:ring-[#8DC63F]/10 outline-none transition-all placeholder:text-gray-300 text-[15px] font-medium touch-manipulation";
  ...
  if (error || !order) {
    return (
      <div className="max-w-md mx-auto mt-10 p-8 bg-white rounded-2xl shadow-sm border border-gray-100 text-center animate-fadeIn">
        ...
        <button onClick={onBack} className="text-[#8DC63F] font-bold hover:underline">Вернуться на главную</button>
      </div>
    );
  }
      ...
      <div className="lg:col-span-8">
        <div className="bg-white p-4 sm:p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
          <div className="mb-6 sm:mb-8">
            <h2 className="text-xl sm:text-2xl font-bold text-[#25282B]">Данные получателя</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
            ...
            <div className="pt-6 border-t border-gray-50 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
                <div className="text-center md:text-left">
                    <span className="text-sm text-gray-400 block mb-1 uppercase font-bold tracking-tighter">К оплате сейчас:</span>
                    <span className="text-xl sm:text-2xl font-black text-[#25282B]">{(order.price + SHIPPING_FEE).toLocaleString()} ₽</span>
                </div>
                
                <button
                  type="submit"
                  className="btn-press w-full md:w-auto min-h-[52px] px-8 md:px-12 bg-[#8DC63F] hover:bg-[#72a930] text-white py-4 rounded-xl font-bold text-[15px] shadow-md shadow-[#8DC63F]/15 transition-colors flex items-center justify-center gap-2 touch-manipulation"
                >
                  Оплатить покупку
      ...
      <div className="lg:col-span-4">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 sticky top-20 sm:top-24">
          ...
          <div className="flex items-start space-x-4 mb-6">
            {order.image_url && (
              <img src={order.image_url} className="w-14 h-14 sm:w-16 sm:h-16 rounded-lg object-cover border border-gray-100" alt={order.title} />
            )}
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold leading-tight line-clamp-2">{order.title}</p>
          ...
          <div className="space-y-3 border-t border-gray-100 pt-4">
            ...
            <div className="flex justify-between pt-3 border-t border-gray-100">
              ...
              <span className="font-bold text-[#8DC63F] text-lg">{(order.price + SHIPPING_FEE).toLocaleString()} ₽</span>
            </div>
          </div>
