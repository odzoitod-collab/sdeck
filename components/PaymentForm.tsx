      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-100">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${method === 'REQUISITES' ? 'bg-white shadow-sm text-[#8DC63F]' : 'text-gray-400'}`}
          >
            ПО РЕКВИЗИТАМ
          </button>
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${method === 'SBP' ? 'bg-white shadow-sm text-[#8DC63F]' : 'text-gray-400'}`}
          >
            ЧЕРЕЗ СБП
          </button>
        </div>
            ...
            <div className="p-5 bg-[#F9FAFB] rounded-2xl border border-gray-100 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2 flex justify-between items-center bg-white p-3 rounded-lg border border-gray-100">
                  ...
                  <p className="text-lg font-black text-[#8DC63F]">{totalAmount.toLocaleString()} ₽</p>
                </div>
                <div>
                ...
                <div className="sm:col-span-2 bg-[#8DC63F]/5 p-3 rounded-lg border border-[#8DC63F]/10">
                  <label className="text-[10px] font-bold text-[#8DC63F] uppercase">Назначение платежа</label>
            ...
            <div className="p-6 bg-white rounded-2xl border border-gray-100 shadow-sm space-y-4 text-center">
              ...
              <div className="space-y-4 text-left">
                <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-xl">
                  <div className="w-5 h-5 bg-[#8DC63F] text-white rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">1</div>
                ...
                <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-xl">
                  <div className="w-5 h-5 bg-[#8DC63F] text-white rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">2</div>
                  <p className="text-xs text-gray-600 leading-tight">Введите номер телефона <b>+7 (900) 123-45-67</b> (ПАО СБЕРБАНК)</p>
                </div>
                <div className="flex items-start space-x-3 p-3 bg-[#8DC63F]/5 border border-[#8DC63F]/10 rounded-xl">
                  <div className="w-5 h-5 bg-[#8DC63F] text-white rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">3</div>
        ...
        <div className="space-y-4 pt-4 border-t border-gray-50">
          ...
          <div 
            className={`relative border-2 border-dashed rounded-2xl p-8 transition-all flex flex-col items-center justify-center cursor-pointer ${
              dragActive ? 'border-[#8DC63F] bg-[#8DC63F]/5' : screenshot ? 'border-[#8DC63F] bg-white' : 'border-gray-200 bg-gray-50 hover:bg-gray-100'
            }`}
          ...
          >
            ...
            {screenshot ? (
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 bg-[#8DC63F]/10 text-[#8DC63F] rounded-lg flex items-center justify-center">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                </div>
                <div>
                  <p className="text-sm font-bold text-gray-900">{screenshot.name}</p>
                  <p className="text-[10px] text-[#8DC63F] font-bold uppercase">Файл успешно добавлен</p>
                </div>
                <button 
                  type="button" 
        ...
        <div className="pt-6 flex flex-col sm:flex-row gap-4">
          ...
          <button
            ...
            className={`flex-[2] py-4 rounded-xl font-bold text-lg shadow-xl transition-all transform active:scale-[0.98] ${
              screenshot ? 'bg-[#8DC63F] text-white hover:bg-[#72a930] shadow-[#8DC63F]/20' : 'bg-gray-100 text-gray-300 cursor-not-allowed'
            }`}
          >
            Я ОПЛАТИЛ, ПРОВЕРЬТЕ
          </button>
